...
from Models.model_t import Net
from Models.data_t import SalObjDataset, test_dataset
...
n_all_parameters = sum(p.numel() for p in model.parameters())
print("**********")
print('number of all params:', n_all_parameters)
print("**********")
print('para_fix:', n_fix_parameters)
print("**********")
print('para_learnable:', n_parameters)
...
print('load data...')
dataset_train = SalObjDataset(train_gt_root, train_depth_root, trainsize=opt.trainsize)
dataset_test = test_dataset(val_gt_root, val_depth_root, opt.trainsize)
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        for i, (gts, depths) in enumerate(train_loader, start=1):
            optimizer.zero_grad()
            ...
            loss_seg = loss1_fusion + dice_loss
            loss = loss_seg
            loss.backward()
            ...
            if i % 50 == 0 or i == total_step or i == 1:
                print(
                    '{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], loss: {:.4f} loss1_fusion: {:0.4f} dice: {:0.4f}'.
                    format(datetime.now(), epoch, opt.epoch, i, total_step, loss.data, loss1_fusion.data,
                           dice_loss.data))
                logging.info(
                    '#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], loss: {:.4f} loss1_fusion: {:0.4f} dice: {:0.4f}'.
                    format(epoch, opt.epoch, i, total_step, loss.data, loss1_fusion.data, dice_loss.data))
        loss_all /= epoch_step
...
def val(test_loader, model, epoch, save_path):
    ...
    with torch.no_grad():
        ...
        for i in range(test_loader.size):
            gt, depth, name, img_for_post = test_loader.load_data()
