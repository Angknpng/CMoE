...
class Net(nn.Module):
    def __init__(self):
        ...
        self.MHSA1_rgb = GMSA_ini(d_model=128)
        #-----------------------------------------Expert_rgb-----------------------------------------------
    ...
    def forward(self, rgb):
        ...
        r4 = self.MHSA4_rgb(flatten4, flatten4).view(flatten4.shape[0], int(np.sqrt(flatten4.shape[1])), int(np.sqrt(flatten4.shape[1])), -1).permute(0, 3, 1, 2).contiguous()
        r3 = self.MHSA3_rgb(flatten3, flatten3).view(flatten3.shape[0], int(np.sqrt(flatten3.shape[1])), int(np.sqrt(flatten3.shape[1])), -1).permute(0, 3, 1, 2).contiguous()
        r2 = self.MHSA2_rgb(flatten2, flatten2).view(flatten2.shape[0], int(np.sqrt(flatten2.shape[1])), int(np.sqrt(flatten2.shape[1])), -1).permute(0, 3, 1, 2).contiguous()
        r1 = fr[0]
        #-----------------------------------------Expert_rgb-----------------------------------------------
